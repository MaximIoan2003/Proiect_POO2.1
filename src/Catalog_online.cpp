...
std::string Catalog_online::nom() const {
    return nume;
}

std::string Catalog_online::adr() const {
    return adresa;
}
...
void Catalog_online::comanda() {
    ...
    int produse = 0;
    int suma = 0;
    std::cout << nom() << ", acum ca ati vazut catalogul, va rog plasati o comanda!" << std::endl;
    ...
    for (const auto& t : termometre) {
        std::cout << t->getFirma() << " -> " << t->getPret() << " ";
    }
    std::cout << std::endl;

    std::cout << "Daca doriti informatii despre bateriilor tastati 1, daca doriti informatii despre temperaturile maxime tastati 2, daca nu doriti sa aflati informatii tastati 3";
    int x;
    std::cin >> x;
    if (x == 1) {
    ...
    if (produse != 0) {
        ...
        if (n == "CONFIRM") {
            ...
            std::cout << nom() << ", va mai asteptam!";
        }
    } else {
        std::cout << "Cosul dumneavoastra este gol!" << std::endl;
    }
}
